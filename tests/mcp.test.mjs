...
test('bundled MCP server exposes tools and recovers a session across server restarts', { timeout: 15000 }, async () => {
  ...
  try {
    ...
    await call(two, 'send_message', { task_id: task.task_id, prompt: 'MOCK:history-hold' }); await idle(two, task.task_id);
    const beforeHistory = await call(two, 'read_task', { task_id: task.task_id, include_output: true });
    const historyError = await two.callTool({ name: 'read_history', arguments: { task_id: task.task_id, timeout_ms: 200 } });
    assert.equal(historyError.isError, true);
    assert.match(historyError.content[0].text, /History replay timed out/);
    const afterHistory = await call(two, 'read_task', { task_id: task.task_id, include_output: true });
    assert.equal(afterHistory.output.text, 'HISTORY_READY');
    assert.equal(afterHistory.task.state, 'idle');
    assert.equal(afterHistory.task.session_id, beforeHistory.task.session_id);
    await call(two, 'send_message', { task_id: task.task_id, prompt: 'MOCK:history-release' }); await idle(two, task.task_id);
    const history = await call(two, 'read_history', { task_id: task.task_id });
    assert.match(history.text, /OLD_REPLAY/);
